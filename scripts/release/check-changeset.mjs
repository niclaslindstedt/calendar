...
// changelog fragment in .changes/unreleased/. This script enforces that, with
// two escape hatches:
//
//   1. Apply the `no-changelog` label to the PR (the workflow passes labels
//      through as $LABELS).
//   2. Touch only files that match the skip-list below (tests, CI, docs,
//      formatter/eslint config, lockfile, etc.).
//
...
try {
  ...
  // Fall through with an empty list. A malformed labels payload shouldn't
  // gate the PR — that would block on infra problems.
}
