...
// fragments waiting in .changes/unreleased/. This is what lets the release
// workflow size a release without a maintainer hand-picking the bump: it
// reads the same front-matter every user-visible PR already drops.
//
// Mapping (each fragment contributes one level; the release takes the HIGHEST
// level across all fragments):
//
...
//
// A change an older build can't survive — a break in the persisted
// CalendarDoc shape that `src/app/migrations.ts` cannot carry forward, or a
// deliberate UX overhaul — is flagged `breaking: true` and forces a major. A
// genuinely breaking removal is therefore `type: Removed` plus
// `breaking: true`, not Removed alone.
//
// When run as a CLI the resolved word is printed to STDOUT on its own line so
// the workflow can capture it directly (`BUMP=$(node compute-bump.mjs)`); all
// diagnostics go to STDERR to keep that capture clean. The pure helpers are
// exported so the bump policy can be unit-tested without touching the
// filesystem.
...
// type → the lowest bump that type implies. `breaking: true` overrides this
// to "major" regardless of type.
...
// implies. Returns "patch" for an empty set — callers that want to refuse an
// empty release check the length themselves.
