...
//
// Two release scripts consume the same fragment files and must parse them
// identically: collate-changelog.mjs (renders them into a CHANGELOG section)
// and compute-bump.mjs (derives the semver bump from their front-matter).
// Keeping the front-matter grammar in one place stops the two from drifting —
// a fragment that collates cleanly is the same fragment the bump is computed
// from. Mirrors the sibling notes app's release scripts.
//
// Fragment format:
//
//   ---
...
//   doc: locales        # optional — links to docs/features/<doc>.md
//   breaking: true      # optional — forces a major bump (see compute-bump.mjs)
//   ---
//
//   One-sentence user-facing summary.
//
// Fragments without front-matter, with an unknown `type`, with a malformed
// front-matter line, or with an empty body fail loudly.
...
// The set of front-matter values that read as "yes" for a boolean flag like
// `breaking`. Anything else (including absent) is false.
...
// A fragment marks a breaking change with `breaking: true` (or yes/1) in its
// front-matter. This is the only signal that escalates a release to a major
// bump.
