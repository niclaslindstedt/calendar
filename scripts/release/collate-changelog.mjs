#!/usr/bin/env node
// Consume .changes/unreleased/*.md fragments, write a new dated section in
// CHANGELOG.md under the [Unreleased] header, delete the fragments. The
// release workflow runs this right after the version bump; contributors can
// run it locally via `make changelog VERSION=X.Y.Z` to preview a release.
//
// Each fragment renders as a single CHANGELOG bullet:
//
//   - **<title>** — <summary> [Learn more](feature:<doc>)
//
// `title` and `doc` are optional. Without a title the summary is the whole
// bullet; with a `doc` slug the bullet gains a "Learn more" link pointing at
// the feature doc (`docs/features/<doc>.md`). Keep the summary to one
// sentence — the long-form explanation belongs in the feature doc, not the
// changelog.
//
// Filename convention: <unix-ts>-<slug>.md. The timestamp gives a
// deterministic lexical sort that loosely tracks commit order; the slug is
// for human scanning. Fragment parsing and validation live in fragments.mjs,
// shared with compute-bump.mjs so the two never disagree on what a valid
// fragment is.
...
for (const file of files) {
  ...
  // Compose the bullet: an optional bold title, the summary, and an optional
  // "Learn more" link to the feature doc. Multi-line bodies are indented
...
// stub stays in place for the next round.
