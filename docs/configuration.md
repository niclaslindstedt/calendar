...
scoped access and an **App folder**, add your deploy origin(s) to the redirect
URIs (the app derives its redirect URI from `origin + pathname`, without a
trailing slash), and put the app key in `VITE_DROPBOX_APP_KEY`.
