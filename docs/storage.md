...
| **Dropbox**      | `<namespace>/calendar.json` in the app folder     | PKCE OAuth; requires `VITE_DROPBOX_APP_KEY`.       |
...
same connection. Switching namespace saves any pending edit first, then loads
the other document; switching backends carries every namespace over the moment
each one is next opened and saved.

**Dropbox is the exception**: it gets a folder per namespace rather than a run
...
The folder is named for the namespace's **slug**, fixed when it was created —
renaming a calendar changes what the switcher says, not where its notes live.
The default namespace is a folder like any other, so nothing sits loose at the
...
Deleting a namespace deletes its Dropbox folder outright; on the other
