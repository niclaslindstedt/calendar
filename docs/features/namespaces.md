...
empties its document in the active backend — on Dropbox, where a namespace is
a folder, the folder goes with it. Either way a namespace re-created under the
...
Dropbox is the one backend where a namespace is a **folder** rather than a
file name: `Apps/nird-calendar/<slug>/calendar.json`, one folder per calendar
at the app folder's root, the default namespace included. Settings → Storage
...
so a second device pointed at the same Dropbox app folder needs its namespaces
