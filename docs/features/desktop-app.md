...
Calendar runs in any browser, and it installs to a home screen or a dock as a
...
[releases page](https://github.com/niclaslindstedt/calendar/releases).
...
- **macOS** — the `.dmg`. The app is signed but not notarized, so the first
  ...
  the message about Calendar and choose **Open Anyway**. macOS remembers after
  that.
...
around it: the same month, week and day views, the same notes, the same settings. Two differences are worth knowing about.
...
the notes you keep in Chrome are not the ones the desktop app opens. Connect the same folder or Dropbox in both (Settings → **Storage**) and they share one calendar.
