...
# Calendar — the desktop app

A desktop wrapper around Calendar for **Windows, macOS and Linux**. It is a thin
...
minimum. Everything Calendar does — the document, the settings, the sync engine —
...
| `src-tauri/src/protocol.rs` | Answering `calendar://` off the bundled `webroot/`            |
...
`cargo test -p calendar-shell` runs the entire decision layer on a machine with a
...
**The origin is the one thing to be careful with.** The user's notes live in
...
`file://` page is an opaque origin, which would leave the notes unable to
survive a launch.
...
| Variable           | Effect                                                               |
| ------------------ | -------------------------------------------------------------------- |
| `CALENDAR_APP_URL` | Load a remote URL instead of the bundled site (the `/preview/` slot) |
| `CALENDAR_WEBROOT` | Serve the site from somewhere else without rebuilding                |
...
identifier (`dev.local.calendar`). What an installed copy is called and the
