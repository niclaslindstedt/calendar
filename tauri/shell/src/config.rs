...
//! shipping: `CALENDAR_APP_URL` points the window at a remote URL instead (the
//! `/preview/` deploy slot, say), and `CALENDAR_WEBROOT` serves a different
...
/// IndexedDB unable to keep a note between launches. A registered scheme
/// gives one stable origin that the notes are keyed to for the life of the
/// install.
pub const APP_SCHEME: &str = "calendar";
...
/// CONSTANT — the origin is what the user's notes are keyed to, so changing
...
pub const WINDOW_TITLE: &str = "Calendar";

/// The window's opening size, and the smallest it may be dragged to.
///
/// The floor is the narrowest the month grid is still readable at — below it
/// the app is not broken, it is just a smaller phone.
pub const DEFAULT_WIDTH: f64 = 1280.0;
pub const DEFAULT_HEIGHT: f64 = 860.0;
