...
impl Site {
    fn new(name: &str) -> Self {
        let root = std::env::temp_dir().join(format!("calendar-webroot-{name}"));
...
fn a_symlink_out_of_the_webroot_is_refused() {
    ...
    let outside = std::env::temp_dir().join("calendar-webroot-symlink-outside.txt");
