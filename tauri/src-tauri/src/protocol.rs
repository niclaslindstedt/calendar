...
//! Answering the `calendar://` scheme — the effects half of
//! `calendar_shell::webroot`, which owns every decision this file acts on.
//!
...
//!
//! `CALENDAR_WEBROOT` overrides both, which is what lets a build serve a site from
//! somewhere else without recompiling — the same escape hatch `CALENDAR_APP_URL`
//! gives for a REMOTE site.

use std::fs;
use std::path::PathBuf;

use calendar_shell::webroot::{content_type_for, resolve_webroot_file};
...
pub fn webroot_dir(app: &AppHandle) -> PathBuf {
    if let Some(override_dir) = std::env::var_os("CALENDAR_WEBROOT") {
...
/// returns a body, not a stream, and the largest thing a Calendar build contains
