...
//! Every decision about that file is `calendar_shell::window_state`'s; this
//! file only asks the window and the monitors.

use calendar_shell::config::{
    app_origin, is_internal_url, remote_app_url, start_url, BRAND_BG, WINDOW_TITLE,
};
use calendar_shell::window_state::{
...
const DEFAULT_WINDOW: WindowState = calendar_shell::window_state::DEFAULT_STATE;
