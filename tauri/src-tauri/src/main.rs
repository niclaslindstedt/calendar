...
//! The Calendar desktop shell — a window showing the bundled site, and nothing
//! else.
//!
...
//! here needed no change to the app at all. Everything Calendar does — the
...
use calendar_shell::config::APP_SCHEME;
use calendar_shell::oauth::{AWAIT_PATH, BEGIN_PATH};
use calendar_shell::webroot::webroot_exists;
...
fn main() {
    tauri::Builder::default()
        ...
        // `calendar_shell::webroot` for the three properties this arrangement is
        ...
        // `calendar_shell::oauth`), which can be minutes, so it answers from a
        ...
        .setup(|app| {
            ...
            if calendar_shell::config::remote_app_url().is_none() && !webroot_exists(&root) {
                let message = format!(
                    "Calendar could not find the app it is meant to show.\n\n\
                     Looked in: {}\n\n\
        ...
        .expect("the Calendar desktop shell could not start");
}
