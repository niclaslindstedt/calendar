...
export const RULES: StoreRules = {
  ...
  brand: {
    ...
    // behaviour does.
    privacyUrl: "https://apps.agilator.se/calendar/privacy/",
    supportUrl: "https://apps.agilator.se/calendar/support/",
  },

  // Calendar ships on the App Store alone. There is no Mac App Store record and
  ...
  apple: {
    // A wall calendar with notes on it. PRODUCTIVITY is where somebody
    // looking for one looks; UTILITIES second because it is small, local and
    // does one thing.
    categories: ["PRODUCTIVITY", "UTILITIES"],

    advisory: {
      // A calendar. Every row is NONE and none of them is a judgement call:
      // the app draws a month, a week and a day, and keeps short notes the
      ...
      // A note on a day is whatever the reader wrote there, which may be a
      // doctor's appointment — but the app records nothing medical of its
      // own, offers no dosages and gives no advice. The question asks what
      // the APP provides, not what a reader may type into a free field.
    ...
    contact: {
      ...
      // REVIEW HAS TO SEE THE CONTACTS PROMPT WORK. Birthdays and name days
      // are the feature that asks for a permission, and a reviewer with an
      // empty address book sees an empty calendar — so the notes say where
      // to turn it on and what it does with what it reads.
      demoRequired: false,
    },
