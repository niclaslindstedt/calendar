...
// `http://localhost:8231…`: an origin it does not have registered, in a
 ...
 *  storage report and the contacts and iCloud requests so none is ever
 *  confused with another. */
export const AUTH_SESSION_REQUEST_TYPE = "calendar-native/auth-session-request";
 ...
 * is `app.config.js`'s `scheme` (the bundle id, `se.agilator.calendar` in the
