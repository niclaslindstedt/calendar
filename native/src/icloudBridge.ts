...
//
//   • a script that defines `window.__calendarICloud`, whose methods post a
...
//
// The wrapper decides nothing about the calendar. It moves opaque files
// between the page and a folder in iCloud Drive: which file a calendar's notes
// live in, what a conflict means and when a save is due are all the web app's,
// in `src/app/storage/` and `src/app/useCalendarStore.ts` against the
// framework's storage adapters. A wrapper that parsed a note would be a second
// implementation of the app's domain.
//
 ...
 *  storage report and the contacts request so the three are never confused. */
export const ICLOUD_REQUEST_TYPE = "calendar-native/icloud-request";

/** The five things the page may ask for — mirrors `ICloudHost`.
 *
 *  Text only: a calendar is one JSON document with nothing binary beside it,
 *  so there is no bytes pair to carry. */
export type ICloudMethod = "status" | "list" | "read" | "write" | "remove";
 ...
 *  in `src/app/storage/icloudHost.ts` — a mismatch is not an error, it is a
 *  storage backend that never appears. */
const HOST_EVENT = "calendar:icloud-host";

/** Where the provider installs itself. Must match `icloudHost.ts`'s
 *  `HOST_PROPERTY`, and for the same reason. */
const HOST_PROPERTY = "__calendarICloud";
...
export const ICLOUD_SCRIPT = `(function () {
  ...
  window.__calendarICloudResolve = function (id, result) {
 ...
 * notes: a note is arbitrary user text, and text is exactly what breaks out
 * of a literal. `JSON.stringify` of the JSON text handles the
...
export function resolveScript(
  ...
  return `(function () {
    try {
      ...
      if (window.__calendarICloudResolve) {
        window.__calendarICloudResolve(answer.id, answer.result);
      }
    } catch (e) {}
  })(); true;`;
}
