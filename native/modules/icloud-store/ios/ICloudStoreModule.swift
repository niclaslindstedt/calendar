...
// calendars are visible in the Files app under "Calendar" rather than hidden
// inside the container. A file the reader can see is a file they can
...
/// The container the calendars sync through. A literal, because Swift cannot
/// read a build variable — kept in step with `../index.ts` and
/// `../../identifiers.js` (which the entitlements and the Files-app
/// declaration read). Changing it after release strands every synced copy in
/// the old container.
private let CONTAINER_ID = "iCloud.se.agilator.calendar"
...
public class ICloudStoreModule: Module {
  ...
  /// but the container holds every one of the reader's calendars, and a store that
  ...
  /// does not own. `.calendar.json.icloud` is `calendar.json`; any other
