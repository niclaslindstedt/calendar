...
// the backend as unavailable there (see ../src/icloud.ts). The widgets' App
// Group is a separate container with its own plugin (./with-widgets.js); the
// two never share a byte.
  ...
  return withInfoPlist(config, (c) => {
    c.modResults.NSUbiquitousContainers = {
      ...
      [CONTAINER_ID]: {
        ...
        // not moved off the device when storage runs low — the notes are the
