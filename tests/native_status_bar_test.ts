...
function fakePage(vars: Record<string, string>) {
  ...
  // An empty store: the reporter patches its writes and reads it for the
  // widgets, neither of which this suite is about.
  class Storage {
    length = 0;
    key() {
      return null;
...
describe("the theme report", () => {
  ...
  it("reports again when the theme engine repaints <html>", () => {
    const vars = { "--page-bg": "#f6f8fa" };
    const page = fakePage(vars);
    page.flush();
    page.posted.length = 0;
    ...
    expect(last.theme.background).toBe("#191724");
  });
