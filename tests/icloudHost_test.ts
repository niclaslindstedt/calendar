...
// The iCloud capability seam (`src/app/storage/icloudHost.ts`).
//
...
// method is a backend that appears in Settings → Storage and then throws on
// the first save; a status that falls through to `ready` is the app pushing a document at
...
import {
  ICLOUD_HOST_METHODS as METHODS,
...
function withInstalled<T>(value: unknown, body: () => T): T {
  ...
  globals.window = { [ICLOUD_HOST_PROPERTY]: value };
  try {
    return body();
  } finally {
...
describe("getICloudHost", () => {
  it("accepts a provider carrying every method", () => {
    ...
    expect(typeof (host as ICloudHost).write).toBe("function");
  });
...
describe("parseICloudStatus", () => {
  ...
  it("falls back to signed-out, never to ready or unavailable", () => {
    ...
    // `unavailable` would hide a backend the reader has already chosen.
...
describe("parseICloudEntries", () => {
  it("keeps the path and the revision token", () => {
    expect(
      parseICloudEntries([{ path: "calendar.json", rev: "17-42" }]),
    ).toEqual([{ path: "calendar.json", rev: "17-42" }]);
  });
  ...
  it("keeps an entry with no revision", () => {
    ...
    expect(parseICloudEntries([{ path: "calendar.work.json" }])).toEqual([
      { path: "calendar.work.json" },
    ]);
  });

  it("drops malformed rows one at a time", () => {
    // One bad row must not hide the calendar the adapter is looking for.
    expect(
      parseICloudEntries([
        { path: "calendar.json" },
        { path: "" },
        ...
        null,
        "calendar.home.json",
        { path: "calendar.work.json", rev: 7 },
      ]),
    ).toEqual([{ path: "calendar.json" }, { path: "calendar.work.json" }]);
  });
...
describe("probeICloudStatus", () => {
  const host = (status: () => Promise<unknown>) =>
    ({ ...goodHost(), status }) as unknown as ICloudHost;

  it("is unavailable with no host", async () => {
    await expect(probeICloudStatus(null)).resolves.toBe("unavailable");
  });

  it("passes a host's answer through its narrowing", async () => {
    await expect(probeICloudStatus(host(async () => "ready"))).resolves.toBe(
      "ready",
    );
    await expect(probeICloudStatus(host(async () => "nope"))).resolves.toBe(
      "signed-out",
    );
  });

  it("never throws: a failing host reads as signed-out", async () => {
    // Signed-out rather than unavailable, so the row stays and the reader
    // can re-check it.
    await expect(
      probeICloudStatus(
        host(async () => {
