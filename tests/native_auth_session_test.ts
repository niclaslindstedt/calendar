...
const REDIRECT = redirectUriFor("se.agilator.calendar");
...
describe("the redirect URI", () => {
  it("is the app's scheme with an oauth path — the string Dropbox must list", () => {
    expect(REDIRECT).toBe("se.agilator.calendar://oauth");
  });

  it("has the bundle id for its scheme, so the store build returns on se.agilator.calendar://oauth", () => {
    // RFC 8252 §7.1: a reverse-DNS scheme no other app can claim. It follows
    // APP_BUNDLE_ID rather than being committed.
    const scheme = schemeFor("se.agilator.calendar");
    expect(scheme).toBe("se.agilator.calendar");
    expect(redirectUriFor(scheme as string)).toBe(REDIRECT);
  });

  it("falls back with the bundle id in a plain checkout", () => {
    expect(schemeFor(undefined)).toBe("dev.local.calendar");
  });
});
...
describe("isAuthSessionRequest", () => {
  ...
  it("ignores messages that are not ours", () => {
    expect(
      isAuthSessionRequest({ ...good, type: "calendar-native/report" }),
    ).toBe(false);
    expect(
      isAuthSessionRequest({ ...good, type: "calendar-native/icloud-request" }),
    ).toBe(false);
    expect(
      isAuthSessionRequest({
        ...good,
        type: "calendar-native/contacts-request",
      }),
    ).toBe(false);
...
describe("authSessionResolveScript", () => {
  it("does not let a provider-controlled URL break out of the script", () => {
    ...
    const nasty = `se.agilator.calendar://oauth?code=");alert(1);//&x=${String.fromCharCode(0x2028)}`;
