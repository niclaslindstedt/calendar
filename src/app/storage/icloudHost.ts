...
// backend is absent, down to its row in Settings → Storage. The app-store
// build's WebView host fills it in (`native/src/icloudBridge.ts`), and so
// could any other host that one day wanted to.
//
// Same rule as `people/contactsHost.ts`: this module never asks whether it is
// running natively, on which platform, or in which build — it asks whether an
// iCLOUD PROVIDER is present, which is a question about capability and not
// about identity. A second host offering the same methods would light the
// same backend up with no change here, and the wrapper stays free to
// disappear without leaving a native-shaped hole in the app.
//
// What the app keeps either way is the DOMAIN. A host moves opaque files; a
// calendar's file name, what a conflict means and when a save is due are all
// decided in `backends.ts`, `paths.ts` and `useCalendarStore.ts` against the
// framework's storage adapters — exactly as they are for a picked local
// folder.
 ...
 *
 *  Five methods, all async, and deliberately no events: iCloud tells the
 *  device when a file changed, but the app already re-reads a calendar when
 *  it is opened, and a host pushing changes would be holding a subscription to
 *  the reader's notes for as long as the app is open.
 *
 *  Text only: a calendar is one JSON document with nothing binary beside it. */
...
export const ICLOUD_HOST_EVENT = "calendar:icloud-host";

/** Where a host installs itself. */
export const ICLOUD_HOST_PROPERTY = "__calendarICloud";
 ...
 *  hide a backend the reader has already chosen). */
...
/** Ask a host for its status without ever throwing: a host that fails on the
 *  cheapest call it has is not one to build a backend on, and is reported as
 *  `signed-out` so the reader can still re-check it. */
export async function probeICloudStatus(
  host: ICloudHost | null,
): Promise<ICloudStatus> {
  if (!host) return "unavailable";
  try {
    return parseICloudStatus(await host.status());
  } catch {
    return "signed-out";
  }
}
 ...
 *  a time rather than failing the whole read — a single bad row must not hide
 *  the calendar the adapter is looking for. */
