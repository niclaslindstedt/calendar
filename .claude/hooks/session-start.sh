...
# Agent UI smoke tests drive a headless Chromium through Playwright
# to screenshot the app while iterating on UI (see AGENTS.md). Playwright is deliberately NOT a
...
# without a manual `npm i` first. Non-fatal: only UI screenshotting needs it, so a
# failure must not break session start.
npm install --no-save --no-audit --no-fund playwright-core@1 \
  || echo "session-start: playwright-core install failed — UI screenshotting" \
          "won't work until it is installed; nothing else needs it." >&2
